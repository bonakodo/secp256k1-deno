...
ffiTest('MuSig2 completes a verified two-party signing flow', () => {
...
ffiTest(
  'MuSig2 supports duplicate keys through distinct participant indexes',
  () => {
    ...
    const publicNonces = secretNonces.map((nonce) =>
...
ffiTest(
  'MuSig2 preserves key order and sorts only on explicit request',
  () => {
...
ffiTest('MuSig2 applies a BIP341 tweak before nonce generation', () => {
...
ffiTest(
  'MuSig2 consumes a nonce before wrong-key and reuse failures',
  () => {
    const flow = twoPartyFlow();
...
ffiTest(
  'MuSig2 secret nonces dispose idempotently without hiding public state',
  () => {
    const flow = twoPartyFlow();
...
ffiTest('MuSig2 secret nonces support using disposal', () => {
  const flow = twoPartyFlow();
  const nonce = flow.secretNonces[0];
  {
    using scoped = nonce;
...
ffiTest(
  'MuSig2 consumes a nonce before wrong aggregation binding fails',
  () => {
    const flow = twoPartyFlow();
...
ffiTest(
  'MuSig2 wire parsing handles malformed peer values without throwing',
  () => {
...
ffiTest(
  'MuSig2 rejects incomplete, duplicate, extra, and empty participant sets',
  () => {
    ...
    assertStateCode(
      () =>
        flow.session.aggregatePartials([
          ...
          {
            participantIndex: 2,
            partialSignature: partials[1].partialSignature,
          },
        ]),
      'extra-participant-index',
    );
...
ffiTest(
  'MuSig2 rejects wrong partial bindings and invalid final inputs',
  () => {
    const flow = twoPartyFlow();
    const partials = signFlow(flow);
    assertEquals(
...
ffiTest(
  'MuSig2 wire values isolate mutable input and output buffers',
  () => {
    const flow = twoPartyFlow();
